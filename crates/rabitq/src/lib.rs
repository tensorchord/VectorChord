...
pub fn code(dims: u32, vector: &[f32]) -> Code {
    ...
    let x_0 = sum_of_abs_x / (sum_of_x_2 * (dims as f32)).sqrt();
    let x_x_0 = dis_u / x_0;
    let fac_norm = (dims as f32).sqrt();
    let max_x1 = 1.0f32 / (dims as f32 - 1.0).sqrt();
    let factor_err = max_x1 * (x_x_0 * x_x_0 - dis_u * dis_u).sqrt();
    let factor_ip = x_x_0 / fac_norm;
    let cnt_pos = vector
        .iter()
    ...
    let cnt_neg = vector
        ...
        .sum::<i32>();
    let factor_cnt = (cnt_pos - cnt_neg) as f32;
...
pub fn process_l2(
    sum: u32,
    (dis_u_2, factor_cnt, factor_ip, factor_err): (f32, f32, f32, f32),
...
pub fn process_dot(
    sum: u32,
    (_, factor_cnt, factor_ip, factor_err): (f32, f32, f32, f32),
    (dis_v_2, b, k, qvector_sum): (f32, f32, f32, f32),
) -> (f32, f32) {
    let e = k * ((2.0 * sum as f32) - qvector_sum) + b * factor_cnt;
