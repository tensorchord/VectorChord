...
use crate::index::vamana::am::{Reloption, ctid_to_key, kv_to_pointer};
use crate::index::vamana::opclass::{Opfamily, opfamily};
use crate::index::vamana::types::VamanaIndexingOptions;
...
pub unsafe extern "C-unwind" fn ambuild(
    ...
    let (vector_options, vamana_options) = unsafe { options(index_relation) };
    if let Err(errors) = Validate::validate(&vector_options) {
        pgrx::error!("error while validating options: {}", errors);
    }
    if let Err(errors) = Validate::validate(&vamana_options) {
        pgrx::error!("error while validating options: {}", errors);
    }
    if !matches!(vector_options.d, DistanceKind::L2S) && vamana_options.index.alpha != 1.0 {
        pgrx::error!("alpha must be 1 if L2 metric is not used");
    }
    ...
    crate::index::vamana::algo::build(vector_options, vamana_options.index, &index);
    reporter.phase(BuildPhase::from_code(BuildPhaseCode::Inserting));
    let cache = vamana_cached::VamanaCached::_0 {};
    if let Some(leader) = unsafe {
        ...
        unsafe {
            ...
            loop {
                pgrx::pg_sys::SpinLockAcquire(&raw mut (*leader.vamanashared).mutex);
                if (*leader.vamanashared).nparticipantsdone == nparticipants {
                    pgrx::pg_sys::SpinLockRelease(&raw mut (*leader.vamanashared).mutex);
                    break;
                }
                pgrx::pg_sys::SpinLockRelease(&raw mut (*leader.vamanashared).mutex);
                pgrx::pg_sys::ConditionVariableSleep(
                    &raw mut (*leader.vamanashared).workersdonecv,
                    pgrx::pg_sys::WaitEventIPC::WAIT_EVENT_PARALLEL_CREATE_INDEX_SCAN as _,
                );
            }
            reporter.tuples_done((*leader.vamanashared).indtuples);
            reporter.tuples_total((*leader.vamanashared).indtuples);
...
impl VamanaLeader {
    pub unsafe fn enter(
        ...
        index_relation: pgrx::pg_sys::Relation,
        isconcurrent: bool,
        cache: vamana_cached::VamanaCached,
    ) -> Option<Self> {
        ...
        let pcxt = unsafe {
            pgrx::pg_sys::CreateParallelContext(
                c"vchord".as_ptr(),
                c"vamana_parallel_build_main".as_ptr(),
                request,
            )
        };
        ...
        let vamanashared = unsafe {
            ...
            pgrx::pg_sys::ConditionVariableInit(&raw mut (*vamanashared).workersdonecv);
            pgrx::pg_sys::SpinLockInit(&raw mut (*vamanashared).mutex);
            vamanashared
        };
        ...
        unsafe {
            pgrx::pg_sys::shm_toc_insert((*pcxt).toc, 0xA000000000000001, vamanashared.cast());
            pgrx::pg_sys::shm_toc_insert((*pcxt).toc, 0xA000000000000002, tablescandesc.cast());
            pgrx::pg_sys::shm_toc_insert((*pcxt).toc, 0xA000000000000003, vamanacached.cast());
        }
...
#[pgrx::pg_guard]
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn vamana_parallel_build_main(
    ...
    let heap_lockmode;
    let index_lockmode;
    if unsafe { !(*vamanashared).isconcurrent } {
    ...
    let heap = unsafe { pgrx::pg_sys::table_open((*vamanashared).heaprelid, heap_lockmode) };
    let index = unsafe { pgrx::pg_sys::index_open((*vamanashared).indexrelid, index_lockmode) };
    let index_info = unsafe { pgrx::pg_sys::BuildIndexInfo(index) };
    unsafe {
        (*index_info).ii_Concurrent = (*vamanashared).isconcurrent;
    }
...
unsafe fn parallel_build(
    ...
    use vamana_cached::VamanaCachedReader;
    let cached = VamanaCachedReader::deserialize_ref(unsafe {
        let bytes = (vamanacached as *const u64).read_unaligned();
        std::slice::from_raw_parts(vamanacached.add(8), bytes as _)
    });
        ...
        VamanaCachedReader::_0(_) => {
            heap.traverse(true, move |(ctid, store)| {
                ...
                unsafe {
                    let indtuples;
                    {
                        pgrx::pg_sys::SpinLockAcquire(&raw mut (*vamanashared).mutex);
                        (*vamanashared).indtuples += 1;
                        indtuples = (*vamanashared).indtuples;
                        pgrx::pg_sys::SpinLockRelease(&raw mut (*vamanashared).mutex);
                    }
                    callback(indtuples);
                }
            });
    ...
    unsafe {
        pgrx::pg_sys::SpinLockAcquire(&raw mut (*vamanashared).mutex);
        (*vamanashared).nparticipantsdone += 1;
        pgrx::pg_sys::SpinLockRelease(&raw mut (*vamanashared).mutex);
        pgrx::pg_sys::ConditionVariableSignal(&raw mut (*vamanashared).workersdonecv);
    }
}
