...
CREATE FUNCTION _vchordrq_logged_samples(regclass)
RETURNS SETOF TEXT
STRICT LANGUAGE c AS 'MODULE_PATHNAME', '_vchordrq_logged_samples_wrapper';

CREATE OR REPLACE FUNCTION vchordrq_logged_queries(regclass)
RETURNS TABLE(
    schema_name NAME,
    table_name NAME,
    column_name NAME,
...
BEGIN
    ...
    query_text := format(
        ...
        FROM
            index_metadata im,
            LATERAL %2$I._vchordrq_logged_samples(%1$s) AS s(vector_text);
        $q$,
        $1::oid,
        ext_schema
    );
