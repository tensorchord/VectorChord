...
use crate::collector::types::{
    COMMAND_REQUEST, Command, MAX_QUERY_LEN, QUERY_LOAD_MQ, QUERY_PUSH_MQ, VCHORD_MAGIC,
};

pub const COMMAND_STRUCT_SIZE: usize =
    size_of::<Command>().next_multiple_of(pgrx::pg_sys::ALIGNOF_BUFFER as _);
...
unsafe fn shm_estimate_size() -> pgrx::pg_sys::Size {
    let mut estimator = pgrx::pg_sys::shm_toc_estimator {
        space_for_chunks: COMMAND_STRUCT_SIZE + MAX_QUERY_LEN * 2,
        number_of_keys: 3,
    };
...
unsafe extern "C-unwind" fn collector_shmem_startup() {
    unsafe {
        ...
        if !found {
            ...
            pgrx::pg_sys::shm_toc_insert(toc, 0, QUERY_PUSH_MQ.cast());

            QUERY_LOAD_MQ =
                pgrx::pg_sys::shm_toc_allocate(toc, MAX_QUERY_LEN).cast::<pgrx::pg_sys::shm_mq>();
            pgrx::pg_sys::shm_toc_insert(toc, 1, QUERY_LOAD_MQ.cast());

            COMMAND_REQUEST =
                pgrx::pg_sys::shm_toc_allocate(toc, COMMAND_STRUCT_SIZE).cast::<Command>();
            pgrx::pg_sys::shm_toc_insert(toc, 2, COMMAND_REQUEST.cast());
        } else {
            let toc = pgrx::pg_sys::shm_toc_attach(VCHORD_MAGIC, pgws);
            QUERY_PUSH_MQ = pgrx::pg_sys::shm_toc_lookup(toc, 0, false) as *mut _;
            QUERY_LOAD_MQ = pgrx::pg_sys::shm_toc_lookup(toc, 1, false) as *mut _;
            COMMAND_REQUEST = pgrx::pg_sys::shm_toc_lookup(toc, 2, false) as *mut _;
        }
    }
}
