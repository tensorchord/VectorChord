...
pub static mut COMMAND_MQ: *mut pgrx::pg_sys::shm_mq = ptr::null_mut();
pub static mut BGWORKER_TO_MAIN_MQ: *mut pgrx::pg_sys::shm_mq = ptr::null_mut();
pub static mut BGWORKER_LATCH: *mut pgrx::pg_sys::Latch = ptr::null_mut();
...
pub const BGWORKER_LATCH_SIZE: usize = size_of::<pgrx::pg_sys::Latch>();

pub const BINCODE_CONFIG: config::Configuration<
    config::LittleEndian,
    config::Varint,
    config::Limit<MAX_QUERY_LEN>,
> = config::standard().with_limit();
...
impl CollectorState {
    pub fn load() -> Option<Self> {
        ...
        path.try_exists().ok().and_then(|exists| {
            ...
            let decode_result =
                decode_from_reader::<CollectorState, _, _>(&mut reader, BINCODE_CONFIG);
